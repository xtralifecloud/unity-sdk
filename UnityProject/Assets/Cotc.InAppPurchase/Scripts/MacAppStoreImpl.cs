...
namespace CotcSdk.InappPurchase {
	...
	class MacAppStoreImpl: IStore {

		private delegate void Delegate(string jsonResult);
		...
		[DllImport("MacCotcInappPurchase")]
		private static extern void CotcInappPurchase_startup();
		[DllImport("MacCotcInappPurchase")]
		private static extern void CotcInappPurchase_listProducts(string json, Delegate onFinished);
		[DllImport("MacCotcInappPurchase")]
		private static extern void CotcInappPurchase_launchPurchase(string json, Delegate onFinished);
		[DllImport("MacCotcInappPurchase")]
		private static extern void CotcInappPurchase_terminatePurchase(string json, Delegate onFinished);

		// GameObjectName is used for callbacks from Java
		public MacAppStoreImpl() {
			CotcInappPurchase_startup();
		}
		...
		Promise<List<ProductInfo>> IStore.GetInformationAboutProducts(List<ConfiguredProduct> products) {
			...
			// Will call back the CotcInappPurchaseGameObject (done like this to prevent AOT issues on iOS)
			CotcInappPurchase_listProducts(interop.ToJson(), (this as IStore).GetInformationAboutProducts_Done);
		...
		Promise<PurchasedProduct> IStore.LaunchPurchaseFlow(Gamer gamer, ProductInfo product) {
			...
			CotcInappPurchase_launchPurchase(interop.ToJson(), (this as IStore).LaunchPurchase_Done);
