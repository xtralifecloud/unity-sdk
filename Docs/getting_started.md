...
The basic principle means that any method of the API will return an @ref CotcSdk.Promise "Promise<Type> object", which promises to give a result of that type in the future. The `Promise` object provides a few methods which will help:
...
Note that the @ref CotcSdk.Promise.Then "Then" and @ref CotcSdk.Promise.Catch "Catch" block return a new promise that can itself be linked to another Then/Catch block. However you should provide a Catch block only after all Then blocks or just before a Done block. The @ref CotcSdk.Promise.Done "Done" method returns no promise, and just tells that it is the last thing that you will do with the result. If you provide a Then block, you may then:
